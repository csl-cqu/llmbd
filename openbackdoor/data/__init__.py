...
def load_dataset(
    ...
    if load and os.path.exists(clean_data_basepath):
        ...
        logger.info(f'load from {clean_data_basepath}')
    ...
    if not test:
        ...
        try:
            train_dataset = processor.get_train_examples(shuffle=True)
        except FileNotFoundError:
            logger.warning("Has no training dataset.")
        try:
            dev_dataset = processor.get_dev_examples(shuffle=shuffle)
        except FileNotFoundError:
    ...
    test_dataset = None
    try:
        test_dataset = processor.get_test_examples(shuffle=shuffle)
    except FileNotFoundError:
...
def load_clean_data(path, split):
    ...
    # clean_data = [(d[0], d[1], 0) for d in data]
    return clean_data
